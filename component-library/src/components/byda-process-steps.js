...
export class BydaProcessSteps extends HTMLElement {
  ...
  buildEventPayload() { return { address: { ...this.state.address }, candidates: this.state.candidates.map((candidate) => ({ ...candidate })), selectedSite: this.state.selectedSite ? { ...this.state.selectedSite } : null, enquiry: { ...this.state.enquiry }, tracking: { ...this.state.tracking }, submitted: this.state.submitted }; }
  ...
  searchCandidates() {
    ...
    this.state.candidates = generateCandidates(this.state.address); this.state.selectedSite = null; this.setDraftTracking();
    this.notice = `${this.state.candidates.length} matches found.`; this.noticeTone = "positive"; this.render(); this.emitComponentEvent("byda-process-change", { reason: "search" });
  }
