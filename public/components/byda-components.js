var BydaComponents = (() => {
  ...
  var DEFAULT_STEPS = ["Find Location", "Enquiry Details", "Review & Track"];
  var DEFAULT_DETAILS = [
...
`;
  ...
  var BydaProcessSteps = class extends HTMLElement {
    ...
    getActivityOptions() {
      const nextOptions = this.state.enquiry.isPlanning ? this.optionsData?.planningActivityTypes : this.optionsData?.excavationActivityTypes;
    ...
    renderTrackingStage() {
      ...
      if (this.state.submitted) {
        ...
        const statusCopy = selectedExistingEnquiry ? "Loaded from the saved enquiry you selected in the search results." : this.state.tracking.message || "The latest BYDA status for this enquiry.";
        return `
        <byda-status-card
      ...
      ${this.state.submitted ? "" : `<div class="button-row"><button class="button success" type="button" data-action="complete" ${boolAttr(this.submissionLoading || !this.isStepTwoComplete())}>${this.submissionLoading ? "Creating..." : USER_COPY.buttons.finish}</button></div>`}
    `;
    }
    renderBody() {
  ...
  __publicField(BydaProcessSteps, "observedAttributes", ["current-step", "debug", "details", "heading", "next-label", "previous-label", "steps"]);
