var BydaComponents = (() => {
  ...
  var ACTIVITY_OPTIONS = ["Conceptual Design", "Engineering Design", "Excavation", "Maintenance"];
  var LOCATION_OPTIONS = ["Private", "Road Reserve", "Footpath", "Nature Strip"];
...
`;
  ...
  function hasAddressSearchInput(address = {}) {
    return Boolean(
      String(address.streetNumber || "").trim() && String(address.streetName || "").trim().length >= 2 && String(address.suburb || "").trim().length >= 2
    );
  }
  ...
  var BydaProcessSteps = class extends HTMLElement {
    ...
    isStepTwoComplete() {
      ...
      return Boolean(this.isStepOneComplete() && e.digStartAt && e.digEndAt);
    }
    isStepThreeComplete() {
    ...
    completeFlow() {
      ...
      this.state.tracking.status = USER_COPY.progress.complete.status;
      this.state.tracking.bydaStatus = USER_COPY.progress.complete.status;
      this.state.tracking.token = this.state.tracking.token || generateToken(this.state.selectedSite?.id || getAddressSeed(this.state.address) || this.state.enquiry.userReference);
      this.state.tracking.completedAt = (/* @__PURE__ */ new Date()).toISOString();
      this.notice = USER_COPY.notices.referenceReady;
      this.noticeTone = "positive";
      this.render();
    ...
    renderTrackingStage() {
      ...
      ${this.state.submitted ? "" : `<div class="button-row"><button class="button success" type="button" data-action="complete" ${boolAttr(!this.isStepTwoComplete())}>${USER_COPY.buttons.finish}</button></div>`}
    `;
    }
    renderBody() {
    ...
    handleClick(event) {
      ...
      if (action === "select-candidate") {
        ...
        this.state.selectedSite = { ...selectedCandidate };
        this.setDraftTracking();
        this.state.enquiry.userReference = "";
        this.ensureAutoUserReference();
