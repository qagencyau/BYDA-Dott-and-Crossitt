var BydaComponents = (() => {
  ...
  __export(index_exports, {
    BydaProcessSteps: () => BydaProcessSteps,
    BydaStatusCard: () => BydaStatusCard,
    defineCustomElements: () => defineCustomElements
  });

  // component-library/src/components/byda-process-steps.js
  var DEFAULT_STEPS = ["Find Location", "Enquiry Details", "Track Enquiry"];
  var DEFAULT_DETAILS = [
  ...
  var LOCATION_TYPES = ["Private", "Road Reserve"];
  var ROAD_LOCATION_OPTIONS = ["Road", "Nature Strip", "Footpath"];
...
`;
  var cloneValue = (value) => JSON.parse(JSON.stringify(value));
  var parseItems = (value) => String(value || "").split("|").map((item) => item.trim()).filter(Boolean);
  var parseInteger = (value, fallback) => {
    const parsed = Number(value);
    return Number.isInteger(parsed) ? parsed : fallback;
  };
  var getTrimmedAttribute = (element, name) => String(element.getAttribute(name) || "").trim();
  var boolAttr = (value) => value ? "disabled" : "";
  var joinParts = (parts) => parts.map((part) => String(part || "").trim()).filter(Boolean).join(" ");
  var escapeHtml = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
  function formatDateLabel(value) {
    if (!value) return "Not set";
    const parsed = new Date(value);
    return Number.isNaN(parsed.getTime()) ? value : new Intl.DateTimeFormat("en-AU", { day: "2-digit", month: "short", year: "numeric" }).format(parsed);
  }
  function formatDateTimeLabel(value) {
    if (!value) return "";
    const parsed = new Date(value);
    return Number.isNaN(parsed.getTime()) ? value : new Intl.DateTimeFormat("en-AU", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(parsed);
  }
  ...
  function getPollIntervalMs(host) {
    ...
    return Number.isFinite(value) && value >= 1e3 ? value : 5e3;
  }
  ...
  function normalizeExistingEnquiry(record = {}) {
    return {
      ...record,
      id: record.id || record.trackingToken || (record.enquiryId ? `enquiry-${record.enquiryId}` : "") || [record.userReference, record.createdAt].filter(Boolean).join("-") || `existing-${Date.now()}`,
  ...
  function hasAddressSearchInput(address = {}) {
    return Boolean(
      String(address.streetNumber || "").trim() && String(address.streetName || "").trim().length >= 2 && String(address.suburb || "").trim().length >= 2 && /^\d{4}$/.test(String(address.postcode || "").trim())
    );
  }
  ...
  var BydaProcessSteps = class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      if (shouldAutoSearchPrefill(this) && this.canGenerateCandidates() && (!this.state.candidates.length || !this.state.existingEnquiries.length)) this.scheduleAddressResultsRefresh({ immediate: true });
    ...
    get value() {
      return cloneValue(this.buildEventPayload());
    }
    set value(nextValue) {
      this.cancelAddressResultsRefresh();
      this.stopStatusPolling();
      ...
      this.authoritiesLoading = false;
      this.submissionLoading = false;
      if (this.isConnected && shouldAutoSearchPrefill(this) && this.canGenerateCandidates() && (!this.state.candidates.length || !this.state.existingEnquiries.length)) this.scheduleAddressResultsRefresh({ immediate: true });
    ...
    reset() {
      ...
      this.authoritiesLoading = false;
      this.submissionLoading = false;
      this.setStepIndex(0, { emitEvent: true, reason: "reset" });
      this.emitComponentEvent("byda-process-change", { reason: "reset" });
    }
    goToStep(nextStep) {
      this.setStepIndex(parseInteger(nextStep, 1) - 1, { emitEvent: true, reason: "programmatic" });
    }
    ...
    isAddressReadonly() {
      return this.hasAttribute("readonly-address");
    }
    isStepOneComplete() {
      return Boolean(this.state.selectedSite);
    }
    isStepTwoComplete() {
      const e = this.state.enquiry;
      return Boolean(this.isStepOneComplete() && e.digStartAt && e.digEndAt && e.activityType && e.locationType && (e.locationType !== "Road Reserve" || e.roadLocation));
    }
    isStepThreeComplete() {
      return Boolean(this.state.submitted);
    }
    canAdvance() {
      return this.stepIndex === 0 ? this.isStepOneComplete() : this.stepIndex === 1 ? this.isStepTwoComplete() : false;
    }
    ...
    async loadAddressResults() {
      ...
      try {
        ...
        if (candidatesResult.status === "fulfilled") {
          this.state.candidates = Array.isArray(candidatesResult.value.sites) ? candidatesResult.value.sites.map((site) => normalizeCandidate(site)) : [];
          this.candidatesError = "";
        } else {
          this.state.candidates = [];
        ...
        if (historyResult.status === "fulfilled") {
          this.state.existingEnquiries = Array.isArray(historyResult.value.enquiries) ? historyResult.value.enquiries.map((enquiry) => normalizeExistingEnquiry(enquiry)) : [];
          this.existingEnquiriesError = "";
        } else {
          this.state.existingEnquiries = [];
    ...
    startStatusPolling(target) {
      ...
      this.statusPollHandle = setInterval(() => {
        ...
        const work = target.trackingToken ? this.loadTrackingStatus(target.trackingToken) : this.loadRemoteEnquiryStatus(target.enquiryId);
        work.catch((error) => {
    ...
    setDraftTracking() {
      ...
      this.state.tracking.status = "Draft";
      this.state.tracking.displayStatus = "Draft";
      this.state.tracking.bydaStatus = "NOT_STARTED";
      this.state.tracking.token = "";
      this.state.tracking.enquiryId = null;
      this.state.tracking.readyUrl = "";
      this.state.tracking.message = "";
      this.state.tracking.completedAt = "";
      this.state.tracking.updatedAt = "";
    }
    buildEnquiryPayload() {
      this.ensureEnquiryDefaults();
      ...
      return {
        ...
        resolvedSite: this.state.selectedSite ? { ...this.state.selectedSite } : void 0,
        userReference: String(this.state.enquiry.userReference || "").trim() || void 0,
        ...
        locationsInRoad: this.state.enquiry.locationType === "Road Reserve" ? [this.state.enquiry.roadLocation].filter(Boolean) : [],
        authorityId: authorityId ? Number(authorityId) : void 0,
        otherAuthorityName: otherAuthorityName || void 0,
        notes: String(this.state.enquiry.notes || "").trim() || void 0,
        userTimezone: Intl.DateTimeFormat().resolvedOptions().timeZone
      };
    }
    searchCandidates() {
      if (!this.canGenerateCandidates()) {
        this.notice = USER_COPY.notices.searchShort;
        this.noticeTone = "neutral";
        this.render();
        return;
      }
      this.syncAddressResults();
      this.notice = "Searching address data.";
    ...
    async completeFlow() {
      if (!this.isStepTwoComplete()) {
        this.notice = USER_COPY.notices.datesNeeded;
        this.noticeTone = "neutral";
        this.setStepIndex(1, { emitEvent: true, reason: "incomplete-enquiry" });
        return;
      }
      ...
      try {
        ...
        this.state.tracking.completedAt = (/* @__PURE__ */ new Date()).toISOString();
        ...
        this.notice = ["ready", "failed"].includes(String(latestStatus.status || "").toLowerCase()) ? USER_COPY.notices.referenceReady : "Enquiry submitted. Status updates will appear here automatically.";
    ...
    renderAddressStage() {
      ...
      const searchResults = this.state.candidates.length ? this.state.candidates : this.state.existingEnquiries.length && enteredAddress ? [{ id: "history-only-result", title: enteredAddress, source: "Address history", copy: "Past enquiries found for this address.", resolvedSite: null }] : [];
        ...
        ${searchResults.map((candidate, candidateIndex) => `
          <div class="search-result ${selectedSite?.id === (candidate.resolvedSite?.id || candidate.id) ? "active" : ""}">
            <div class="search-result-head">
              ...
              ${candidate.resolvedSite ? `<button class="button ${selectedSite?.id === (candidate.resolvedSite?.id || candidate.id) ? "primary" : ""}" type="button" data-action="select-candidate" data-candidate-id="${escapeHtml(candidate.id)}">${selectedSite?.id === (candidate.resolvedSite?.id || candidate.id) ? USER_COPY.buttons.chosen : USER_COPY.buttons.choose}</button>` : ""}
            </div>
      ...
      const addressSummary = readonlyAddress ? `
        ...
        return `
              <div class="readonly-field ${ok ? "" : "missing"}">
                <span class="readonly-icon" aria-hidden="true">${ok ? "\u2713" : "\xD7"}</span>
                <span>
    ...
    renderEnquiryStage() {
      ...
      ${this.optionsLoading ? `<div class="empty-state wide"><span class="field-label">Options</span><span class="summary-help">${USER_COPY.notices.optionsLoading}</span></div>` : this.optionsError ? `<div class="empty-state wide"><span class="field-label">Options</span><span class="summary-help">${escapeHtml(this.optionsError)}</span></div>` : ""}
      ${this.authoritiesLoading ? `<div class="empty-state wide"><span class="field-label">${USER_COPY.details.authority}</span><span class="summary-help">${USER_COPY.notices.authoritiesLoading}</span></div>` : this.authoritiesError ? `<div class="empty-state wide"><span class="field-label">${USER_COPY.details.authority}</span><span class="summary-help">${escapeHtml(this.authoritiesError)}</span></div>` : ""}
      <div class="form-grid">
    ...
    renderTrackingStage() {
      ...
      const statusLabel = selectedExistingEnquiry ? getExistingEnquiryStatusLabel(selectedExistingEnquiry) : this.state.tracking.displayStatus || this.state.tracking.status || this.state.tracking.bydaStatus || "Submitted";
      const statusKey = selectedExistingEnquiry ? getExistingEnquiryStatusKey(selectedExistingEnquiry) : String(this.state.tracking.status || this.state.tracking.displayStatus || this.state.tracking.bydaStatus || "submitted").toLowerCase();
      ...
      const cardWorkDates = selectedExistingEnquiry ? formatWorkDatesLabel(selectedExistingEnquiry.digStartAt, selectedExistingEnquiry.digEndAt) : formatWorkDatesLabel(this.state.enquiry.digStartAt, this.state.enquiry.digEndAt);
      const cardUpdatedAt = selectedExistingEnquiry ? formatDateTimeLabel(selectedExistingEnquiry.updatedAt || selectedExistingEnquiry.createdAt) || "Not available" : formatDateTimeLabel(this.state.tracking.updatedAt || this.state.tracking.completedAt) || "Not available";
      const cardDescription = selectedExistingEnquiry ? "You selected a past enquiry from the search results. The BYDA status below reflects that saved enquiry." : this.state.tracking.message || "The enquiry has been submitted. The latest BYDA status is shown below.";
      const statusValue = selectedExistingEnquiry ? selectedExistingEnquiry.bydaStatus || statusLabel : this.state.tracking.bydaStatus || "Not available";
      const readyUrl = selectedExistingEnquiry ? selectedExistingEnquiry.readyUrl : this.state.tracking.readyUrl;
      const statusCopy = selectedExistingEnquiry ? selectedExistingEnquiry.message || "Loaded from the saved enquiry you selected in the search results." : this.state.tracking.message || "The latest BYDA status for this enquiry.";
      return `
      <byda-status-card
    ...
    renderBody() {
      return this.stepIndex === 0 ? this.renderAddressStage() : this.stepIndex === 1 ? this.renderEnquiryStage() : this.renderTrackingStage();
    }
    ...
    restoreFocusSnapshot(snapshot) {
      ...
      if ((nextField instanceof HTMLInputElement || nextField instanceof HTMLTextAreaElement) && typeof snapshot.selectionStart === "number" && typeof snapshot.selectionEnd === "number") {
    ...
    handleFieldChange(event) {
      ...
      this.notice = "";
      this.noticeTone = "neutral";
      if (target.dataset.scope === "address" && this.stepIndex === 0) {
        this.render();
        this.emitComponentEvent("byda-process-change", { reason: "address-input" });
        return;
      }
      this.syncFooterState();
      this.syncDebugState();
      this.emitComponentEvent("byda-process-change", { reason: "field-change" });
    }
    handleClick(event) {
      ...
      if (action === "previous") {
        this.notice = "";
        this.noticeTone = "neutral";
        this.setStepIndex(this.stepIndex - 1, { emitEvent: true, reason: "previous" });
        return;
      }
      if (action === "next") {
        if (!this.canAdvance()) {
          this.notice = this.stepIndex === 0 ? "Choose a location to continue." : USER_COPY.notices.datesNeeded;
          this.noticeTone = "neutral";
          this.render();
          return;
        }
        ...
        this.setStepIndex(this.stepIndex + 1, { emitEvent: true, reason: "next" });
        return;
      }
      if (action === "reset") {
        this.reset();
      ...
      if (action === "select-candidate") {
        ...
        this.state.selectedSite = { ...selectedCandidate.resolvedSite };
        this.setDraftTracking();
        this.state.enquiry.userReference = "";
        this.ensureAutoUserReference();
        this.notice = USER_COPY.notices.locationSelected;
        this.noticeTone = "positive";
        this.render();
        void this.loadAuthorities(this.state.selectedSite);
    ...
    render() {
      ...
      this.elements.next.textContent = getTrimmedAttribute(this, "next-label") || (this.stepIndex === 1 && !this.state.submitted ? this.submissionLoading ? "Creating..." : USER_COPY.buttons.finish : USER_COPY.buttons.continue);
  ...
  __publicField(BydaProcessSteps, "tagName", "byda-process-steps");
  __publicField(BydaProcessSteps, "observedAttributes", ["current-step", "debug", "details", "heading", "next-label", "previous-label", "readonly-address", "steps"]);

  // component-library/src/components/byda-status-card.js
...
`;
  function getTrimmedAttribute2(element, name) {
  ...
  var BydaStatusCard = class extends HTMLElement {
    constructor() {
      super();
      this.attachShadow({ mode: "open" });
    ...
    render() {
      const eyebrow = getTrimmedAttribute2(this, "eyebrow") || "Reusable UI";
      const heading = getTrimmedAttribute2(this, "heading") || "BYDA component starter";
      const status = getTrimmedAttribute2(this, "status") || "Ready";
      const description = getTrimmedAttribute2(this, "description") || "Ship this as a single Web Component and reuse it in WordPress, React, Angular, or a plain HTML page.";
      ...
      const metaRows = [
        {
          label: "Reference",
          value: getTrimmedAttribute2(this, "reference")
        },
        {
          label: "Location",
          value: getTrimmedAttribute2(this, "location")
        },
        {
          label: "Work dates",
          value: getTrimmedAttribute2(this, "work-dates")
        },
        {
          label: "Updated",
          value: getTrimmedAttribute2(this, "updated-at")
        }
      ].filter((row) => row.value);
      this.elements.meta.hidden = metaRows.length === 0;
      this.elements.meta.innerHTML = metaRows.map(
        (row) => `
          <div class="meta-row">
  ...
  __publicField(BydaStatusCard, "tagName", "byda-status-card");
  __publicField(BydaStatusCard, "observedAttributes", [
    "description",
    "eyebrow",
    "heading",
    "location",
  ...
  // component-library/src/index.js
  function defineCustomElements(registry = globalThis.customElements) {
    if (!registry) {
      return;
    }
...
//# sourceMappingURL=byda-components.js.map
